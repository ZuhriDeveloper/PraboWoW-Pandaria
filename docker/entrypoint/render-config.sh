...
VARS='${DB_HOST} ${DB_PORT} ${DB_USER} ${DB_PASSWORD} ${DB_AUTH} ${DB_CHARACTERS} ${DB_WORLD} ${REALM_ID} ${REALM_PORT} ${AUTH_PORT} ${DATA_DIR_CONTAINER} ${LOGS_DIR_CONTAINER} ${SQL_DIR_CONTAINER} ${WORLD_DUMP_CONTAINER} ${RATE_XP} ${RATE_DROP_ITEM_COMMON} ${RATE_DROP_ITEM_QUALITY} ${RATE_DROP_ITEM_REFERENCED} ${RATE_DROP_MONEY} ${RATE_HONOR} ${RATE_REPUTATION} ${RATE_PROFESSION} ${HEIRLOOM_PRICE_GOLD} ${PLAYERBOTS_ENABLE} ${PLAYERBOTS_MAX_BOTS} ${PLAYERBOTS_ACCOUNTS} ${PLAYERBOTS_ACCOUNT_PASSWORD} ${PLAYERBOTS_JOIN_LFG}'
